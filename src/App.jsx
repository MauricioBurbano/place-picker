...
const storedPlaces = (
  JSON.parse(localStorage.getItem("storedPlaces")) || []
).map((id) => AVAILABLE_PLACES.find((place) => place.id === id));

function App() {
  const selectedPlace = useRef();
  const [pickedPlaces, setPickedPlaces] = useState(storedPlaces);
  const [places, setSortedPlaces] = useState([]);
  ...
  return (
    <>
      <Modal open={modalIsOpen} updateOpen={handleStopRemovePlace}>
        <DeleteConfirmation
      ...
      <main>
        ...
        <Places
          ...
          places={places}
          fallbackText="Finding places nearest to your location..."
